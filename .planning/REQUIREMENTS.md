...
- [x] **PG-01**: `PostgresAdapter` connects via JDBC using per-workspace cached HikariCP pool (maxPoolSize=2, idleTimeout=10m, maxLifetime=30m)
- [x] **PG-02**: `introspectSchema()` queries INFORMATION_SCHEMA for tables/columns/types
- [x] **PG-03**: `fetchRecords()` uses `WHERE updated_at > :lastSyncCursor` with cursor-based pagination
- [x] **PG-04**: Fallback cursor strategy for tables without `updated_at`: primary-key-based comparison (inserts only)
- [x] **PG-05**: Postgres column values mapped directly to `EntityAttributePrimitivePayload` (bypass SchemaMappingService)
- [x] **PG-06**: `syncMode()` returns `POLL`
- [x] **PG-07**: FK constraint introspection creates `RelationshipDefinitionEntity` between mapped entity types (best-effort)
...
| PG-01 | Phase 3 | Complete |
| PG-02 | Phase 3 | Complete |
| PG-03 | Phase 3 | Complete |
| PG-04 | Phase 3 | Complete |
| PG-05 | Phase 3 | Complete |
| PG-06 | Phase 3 | Complete |
