...
export function createHandleFileChange(params: FileUploadHandlerParams) {
  return async (e: React.ChangeEvent<HTMLInputElement>) => {
    const { container, dispatch, state, toast, setIsUploading, fileInputRef, onUploadImage } =
      params;
    ...
    try {
      ...
      const targetId = state.activeNodeId || container.children[container.children.length - 1]?.id;
      if (targetId) {
  ...
  return async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (targetId) {
        ...
        dispatch(EditorActions.insertNode(flexContainer, container.id, 'append'));
      }
 ...
 */
export function createHandleFreeImageFileChange(params: FileUploadHandlerParams) {
  return async (e: React.ChangeEvent<HTMLInputElement>) => {
    const { dispatch, toast, setIsUploading, fileInputRef, onUploadImage } = params;
    const file = e.target.files?.[0];
    if (!file) return;
