...
export const BlockEnvironmentProvider: React.FC<BlockEnvironmentProviderProps> = ({
  ...
  const insertBlock = useCallback(
    (child: BlockNode, parentId: string, index: number | null = null): string => {
      setEnvironment((prev) => {
        ...
        if (!success) {
          console.warn(`Failed to insert block ${child.block.id} into parent ${parentId}`);
          return prev;
        }
  ...
  return (
    <BlockEnvironmentContext.Provider value={value}>{children}</BlockEnvironmentContext.Provider>
  );
};
