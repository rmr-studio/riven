        ...
        if (line.children && line.children.length > 0) {
          lineContent = line.children
            .map((child) => {
              ...
              const isHexColor = child.className && child.className.startsWith('#');
          ...
          lineContent = isCodeBlock ? escapeHTML(line.content || '') : line.content || '';
        }
  ...
  if (hasChildren) {
    return textNode
      .children!.map((child) => {
        ...
        const childContent = isCodeBlock ? escapeHTML(child.content || '') : child.content || '';

        // If it's a link
        if (child.href) {
          const linkClasses = ['underline cursor-pointer', classes].filter(Boolean).join(' ');
        ...
        if (child.elementType) {
          ...
          const combinedClasses = [elementClasses, classes].filter(Boolean).join(' ');
          return `<span class="${combinedClasses}"${styleAttr}>${childContent}</span>`;
        }
