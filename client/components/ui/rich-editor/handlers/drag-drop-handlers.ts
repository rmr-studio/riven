...
import { ContainerNode, EditorNode, isContainerNode, isTextNode, TextNode } from '../types';
...
export function createHandleDragOver(
  params: Omit<DragDropHandlerParams, 'toast' | 'setIsUploading' | 'onUploadImage'>,
) {
  return (e: React.DragEvent, nodeId: string) => {
    ...
    const draggingResult = draggingNodeId ? findNodeAnywhere(draggingNodeId, container) : null;

    if (!targetResult) return;

    const targetNode = targetResult.node;
    const draggingNode = draggingResult?.node;
    const isTargetImage = isTextNode(targetNode) && (targetNode as TextNode).type === 'img';
    const isDraggingImage =
      draggingNode && isTextNode(draggingNode) && (draggingNode as TextNode).type === 'img';
    ...
    if (isTargetImage && isDraggingImage) {
      ...
      if (inSameFlexContainer) {
        ...
        const parent = draggingResult?.parentId
          ? (container.children.find((c) => c.id === draggingResult.parentId) as ContainerNode)
          : null;

        if (parent) {
          const dragIndex = parent.children.findIndex((c) => c.id === draggingNodeId);
    ...
    if (draggingNodeId) {
      ...
      const draggedIndex = container.children.findIndex((n) => n.id === draggingNodeId);
    ...
    e.dataTransfer.dropEffect = draggedNodeId || draggingNodeId ? 'move' : 'copy';
    ...
    if (draggedNodeId && draggingNodeId) {
      ...
      if (dropPosition === 'left' || dropPosition === 'right') {
        ...
        if (isTextNode(draggingNode) && isTextNode(targetNode)) {
          ...
          if (draggingResult.parentId && draggingResult.parent?.attributes?.layoutType === 'flex') {
          ...
          const targetRootIndex = container.children.findIndex(
            (n) =>
              n.id === nodeId ||
              (isContainerNode(n) && (n as ContainerNode).children.some((c) => c.id === nodeId)),
          );
      ...
      if (draggingResult.parentId && draggingResult.parent) {
        const parent = draggingResult.parent;
        const remainingChildren = parent.children.filter((c) => c.id !== draggingNodeId);

        const insertPos =
          dropPosition === 'before' || dropPosition === 'after' ? dropPosition : 'after';
        ...
        if (remainingChildren.length === 1) {
          ...
          if (isTargetTheFlexContainer) {
            ...
            if (referenceNodeId) {
              ...
              actions.push(
                EditorActions.insertNode(remainingChildren[0], referenceNodeId, referencePosition),
              );

              // Now insert the dragged node next to the remaining child
              actions.push(
                EditorActions.insertNode(draggingNode, remainingChildren[0].id, insertPos),
              );
      ...
      const draggingNodeAtRoot = container.children.find((n) => n.id === draggingNodeId);
