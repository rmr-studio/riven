      ...
      {readOnly ? (
        ...
        <ContextMenu>
          ...
          <ContextMenuContent className="w-56">
            <ContextMenuItem onClick={() => setIsDialogOpen(true)} className="gap-2">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="overflow-y-auto sm:max-w-[450px]">
            <DialogHeader>
              ...
              <DialogDescription>Choose a background color for this block</DialogDescription>
            </DialogHeader>
            ...
            <Tabs defaultValue="preset" className="w-full">
                  ...
                  {presetColors.map((color) => (
                    <button
                      ...
                      style={{
                        ...
                        backgroundPosition:
                          color.hex === 'transparent' ? '0 0, 5px 5px' : 'initial',
                      }}
                    >
                      <span className="text-xs font-medium text-foreground/60">{color.name}</span>
                    </button>
                  ))}
                </div>
              </TabsContent>
              ...
              <TabsContent value="custom" className="space-y-3 pt-4">
                <ShadcnColorPicker
                  ...
                  className="w-full rounded-md border bg-background p-3 shadow-sm"
                >
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 flex-1 items-center gap-2">
                    ...
                    />
                    <span className="truncate font-mono text-xs font-medium">{displayColor}</span>
                  </div>
                  <Button onClick={handleApplyCustomColor} size="sm" className="shrink-0">
                    Apply
                  </Button>
                </div>
              </TabsContent>
