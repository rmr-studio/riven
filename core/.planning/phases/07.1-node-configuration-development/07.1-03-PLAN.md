...
must_haves:
  truths:
    ...
    - "ConfigValidationService exposes isTemplate() helper method"
  artifacts:
    - path: "src/main/kotlin/riven/core/service/workflow/ConfigValidationService.kt"
      provides: "Config validation service with isTemplate() helper"
      contains: "fun isTemplate"
...
<task type="auto">
  <name>Task 1: Add isTemplate helper to ConfigValidationService</name>
  <files>src/main/kotlin/riven/core/service/workflow/ConfigValidationService.kt</files>
  <action>
Add a public helper method to ConfigValidationService that exposes template detection capability. This allows config classes to check if a value is a template without accessing the private templateParserService directly.

Add this method to ConfigValidationService:
...
This method should be added after the existing public methods. The templateParserService remains private, but its isTemplate functionality is now exposed through ConfigValidationService.
  </action>
  <verify>
```bash
./gradlew compileKotlin --quiet 2>&1 | head -20
```
ConfigValidationService compiles with new isTemplate() method.
  </verify>
  <done>ConfigValidationService has public isTemplate() helper method</done>
</task>

<task type="auto">
  <name>Task 2: Refactor WorkflowHttpRequestActionConfig with typed fields</name>
...
- Uses `validationService.isTemplate()` instead of accessing templateParserService directly
...
<verification>
1. ConfigValidationService has isTemplate() helper method
2. Both HTTP_REQUEST and CONDITION configs compile without errors
3. Each config has typed fields instead of `config: Map/JsonObject`
4. Each config has `validate()` method
5. HTTP_REQUEST validates method enum
6. CONDITION can validate expression syntax
7. `./gradlew compileKotlin` succeeds
</verification>

<success_criteria>
- ConfigValidationService has public isTemplate() method
