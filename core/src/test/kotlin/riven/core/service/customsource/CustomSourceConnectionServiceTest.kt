...
/**
 * Unit tests for [CustomSourceConnectionService].
 *
...
@SpringBootTest(
    classes = [
        ...
        CustomSourceConnectionServiceTest.TestConfig::class,
        CustomSourceConnectionService::class,
    ],
)
@WithUserPersona(
...
class CustomSourceConnectionServiceTest {
    ...
    @Autowired private lateinit var service: CustomSourceConnectionService
    ...
    private fun validRequest() = CreateCustomSourceConnectionRequest(
    ...
    fun `update with name only skips the gate chain`() {
        ...
        service.update(workspaceId, id, UpdateCustomSourceConnectionRequest(name = "renamed"))
    ...
    fun `update with host change re-runs gate chain and re-encrypts with merged payload`() {
        ...
        service.update(
            workspaceId, id,
            UpdateCustomSourceConnectionRequest(host = "new.example.com"),
        )
    ...
    fun `update with password change re-runs gates and rotates ciphertext`() {
        ...
        service.update(
            workspaceId, id,
            UpdateCustomSourceConnectionRequest(password = "new-pw"),
        )
    ...
    fun `update on missing entity throws NotFoundException`() {
        ...
        assertThrows<NotFoundException> {
            service.update(workspaceId, id, UpdateCustomSourceConnectionRequest(name = "x"))
        }
    }
