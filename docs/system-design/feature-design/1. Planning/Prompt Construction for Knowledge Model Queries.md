...
### Problem Statement
In order to fully utilise an LLM's reasoning capabilities to perform cross domain intelligent analysis. The prompt provided must be constructed query itself, alongside all relevant metadata, vectors and entity types that are needed.
A templating system will be required that will inject schema definitions, retrieved context and the original query itself, into a concise prompt
### Proposed Solution
