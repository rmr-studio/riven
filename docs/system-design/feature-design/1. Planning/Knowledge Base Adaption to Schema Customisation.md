...
### Problem Statement
- One of the biggest features of the application is the compounding contextual knowledge that is accumulated as data is continuously added to each entity model. As more vector embeddings can be created, imbued with semantic metadata to explain each attribute and relationship. 
- However, given the nature of flexible modelling within the application, a significant overhaul to the environment's data structure could leave existing embeddings, and the RAG itself unable to provide relevant data to fit the needs of the new data structure
- The AI needs to reconcile data structure changes against its existing understanding
### Proposed Solution
- One of the simplest new features mentioned inside [[Semantically Imbued Entity Attributes]] Is to include the ability for the user to add semantic context to each new attribute or relationship. 
- However to handle the modification of the knowledge layer during
	- Attribute/Relationship Deletion
	- Relationship linking modifications (ie. Changing what a relationship links to)
	- Removal of an entity type all together
	- Updating the metadata/name of an entity type
