...
flowchart TB
    ...
    subgraph "TemplateParserService"
        TPS_PARSE[parse]
        TPS_DETECT[Detect #123;#123; #125;#125;]
...
flowchart LR
    subgraph "Node A"
        A_EXEC[Execute]
        A_OUT[Output: &#123; email: 'a#64;b#46;com' #125;]
    end
    ...
    subgraph "Node B (depends on A)"
        B_CONFIG[Config:<br/>to: '#123;#123; steps.node_a.output.email #125;#125;']
        B_RESOLVE[InputResolver]
        B_RESOLVED[Resolved:<br/>to: a#64;b&#46;com]
        B_EXEC[Execute]
    end
